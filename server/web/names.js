(function() {
  ...
  $.fn.makeNameAwesomenessDetector = function(initialName, locale) {
    ...
    formatToDecimalPlaces = function(f, decimalPlaces) {
      ...
      s = s.replace(/[,\.]/, locale.number.decimal);
      while (/\d{4}/.test(s)) {
        s = s.replace(/(\d)(\d{3})\b/, "$1" + locale.number.thousands_separator + "$2");
      }
      return s;
    };
    ...
    preprocessData = function(data) {
      var city, i, item, items, name, params, total_km, _i, _len, _len2, _ref, _results;
      data.extra = {};
      if (data['quebec-streets'] != null) {
        ...
        data.extra['quebec-streets'] = {
          total_km: total_km
        };
      }
      if (data['montreal-metro-stations'] != null) {
        items = data['montreal-metro-stations'];
        _results = [];
        for (i = 0, _len2 = items.length; i < _len2; i++) {
          item = items[i];
          item.line_translated = locale.metro_stations[item.line];
          if (i < items.length - 2) {
            _results.push(item.separator = locale.sentence.connector);
          } else if (i === items.length - 2) {
            _results.push(item.separator = locale.sentence.last_connector);
          } else {
            _results.push(item.separator = '');
          }
        }
        return _results;
      }
